...
TEST_CASE("Check Addition") {
  ...
  SECTION("basic eval with list add") {
    ...
    CHECK(std::get<int>(yall::eval(e, ev)) == 3);
  }

  SECTION("basic eval with list add on empty") {
    environment ev;
    auto e = read_string("(+)");
    CHECK_THROWS(std::get<int>(yall::eval(e, ev)));
  }
...
TEST_CASE("Check Subtraction") {
  SECTION("simple subtraction") {
    environment ev;
    auto e = read_string("(- 1 2)");
    CHECK(std::get<int>(yall::eval(e, ev)) == -1);
  }
  SECTION("wrong type ") {
    environment ev;
    auto e = read_string("(- (quote ()) 1)");
...
TEST_CASE("Check Multiplication") {
  SECTION("simple multiplication") {
    ...
    CHECK(std::get<int>(yall::eval(e, ev)) == 6);
  }
  SECTION("wrong type ") {
    environment ev;
    auto e = read_string("(* (quote ()) 1)");
...
TEST_CASE("Check equality") {
  ...
  SECTION("basic eval with equality true") {
    ...
    CHECK(std::get<boolean>(yall::eval(e, ev)).value == true);
  }
  SECTION("basic eval with equality false") {
    expression e = read_string("(= 3 2)");
    CHECK(std::get<boolean>(yall::eval(e, ev)).value == false);
  }
  SECTION("wrong type ") {
...
TEST_CASE("basic eval with multiplication") {
  ...
  SECTION("Quote no args") {
    ...
    CHECK_THROWS(std::get<boolean>(yall::eval(e, ev)).value);
  }
  ...
  SECTION("nested quoting") {
    ...
    CHECK(to_string(yall::eval(e, ev)) == "(quote (1 2))");
  }
}
...
TEST_CASE("Check Lamba") {
  ...
  SECTION("Check basic lambda example args and uses it") {
    ...
    auto tmp = std::get<expression::lisp_function>(yall::eval(e, ev));
  ...
  SECTION("Check nested quoting in lambda") {
    ...
    environment env;
    to_string(yall::eval(e, env));
    e = read_string("(x)");
    CHECK(to_string(yall::eval(e, env)) == "a");
  }
  ...
  SECTION("Check nested quoting in lambda double def") {
    ...
    CHECK(to_string(yall::eval(e, env)) == "(print this)");
  }
  SECTION("Check let inside definition with lambda") {
    ...
    CHECK(to_string(yall::eval(e, env)) == "401");
  }
  SECTION("Check proposal let with lambda") {
    ...
    to_string(yall::eval(e, env));
    e = read_string(R"((x 2))");
    CHECK(to_string(yall::eval(e, env)) == "3");
  }
...
TEST_CASE("Check Let") {
  ...
  SECTION("Check proposal nested let") {
    auto e = read_string(R"((let ((x 1)
                      ...
                      (let ((x 5))
                         (+ x y))))");
    CHECK(to_string(yall::eval(e, ev)) == "14");
  }
}
...
TEST_CASE("Check eq") {
  ...
  SECTION("Check eq on symbol") {
    ...
    CHECK(to_string(yall::eval(e, env)) == "#t");
  }
...
TEST_CASE("basic eval with nested lists") {
  ...
  CHECK(std::get<int>(yall::eval(e, ev)) == 13);
}
...
TEST_CASE("Check cdr") {
  ...
  SECTION("cdr on cons cell") {
    ...
    CHECK(to_string(yall::eval(e, env)) == "(2 3)");
  }
  SECTION("cdr on cons cell") {
  ...
  SECTION("Check cdr on single element list") {
    ...
    CHECK(to_string(yall::eval(e, env)) == "()");
  }
...
TEST_CASE("Check Append") {
  ...
  SECTION("Append 2 long lists") {
    ...
    CHECK(to_string(yall::eval(e, env)) == "(1 2 1 10 2)");
  }
}
...
TEST_CASE("Check Cons") {
  ...
  SECTION("Cons element into longer list") {
    ...
    CHECK(to_string(yall::eval(e, env)) == "(1 2 3)");
  }
